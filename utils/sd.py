...
from . import model_patcher as model_patcher_
# import comfy.lora
...
class CLIP:
    def __init__(self, target=None, embedding_directory=None, no_init=False):
        ...
        self.patcher = model_patcher_.ModelPatcher(self.cond_stage_model, load_device=load_device, offload_device=offload_device)
        self.layer_idx = None
    ...
    def __init__(self, sd=None, device=None, config=None, dtype=None):
        ...
        if config is None:
            if "decoder.mid.block_1.mix_factor" in sd:
                ...
                self.first_stage_model = AutoencodingEngine(regularizer_config={'target': "utils.ldm.models.autoencoder.DiagonalGaussianRegularizer"},
                                                            encoder_config={'target': "utils.ldm.modules.diffusionmodules.model.Encoder", 'params': encoder_config},
                                                            decoder_config={'target': "utils.ldm.modules.temporal_ae.VideoDecoder", 'params': decoder_config})
            elif "taesd_decoder.1.weight" in sd:
                self.first_stage_model = taesd.taesd.TAESD()
        ...
        self.patcher = model_patcher_.ModelPatcher(self.first_stage_model, load_device=self.device, offload_device=offload_device)
...
def load_gligen(ckpt_path):
    ...
    if model_management.should_use_fp16():
        model = model.half()
    return model_patcher_.ModelPatcher(model, load_device=model_management.get_torch_device(), offload_device=model_management.unet_offload_device())
...
def load_checkpoint(config_path=None, ckpt_path=None, output_vae=True, output_clip=True, embedding_directory=None, state_dict=None, config=None):
    ...
    return (model_patcher_.ModelPatcher(model, load_device=model_management.get_torch_device(), offload_device=offload_device), clip, vae)
...
def load_checkpoint_guess_config(ckpt_path, output_vae=True, output_clip=True, output_clipvision=False, embedding_directory=None, output_model=True):
    ...
    if output_model:
        model_patcher = model_patcher_.ModelPatcher(model, load_device=load_device, offload_device=model_management.unet_offload_device(), current_device=inital_load_device)
...
def load_unet_state_dict(sd): #load unet in diffusers format
    ...
    return model_patcher_.ModelPatcher(model, load_device=load_device, offload_device=offload_device)
